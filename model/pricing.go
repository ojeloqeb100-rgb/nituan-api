...
type Pricing struct {
	...
	VideoPrices            *billing_setting.VideoResolutionPrices `json:"video_prices,omitempty"`
...
func updatePricing() {
	...
	for model, groups := range modelGroupsMap {
		...
		} else if billingMode == billing_setting.BillingModeVideo {
			if videoPrices, ok := billing_setting.GetVideoPrices(model); ok {
				pricing.BillingMode = billingMode
				pricing.VideoPrices = &videoPrices
				pricing.QuotaType = 1
			}
		}
